    ...
    useEffect(() => {
        const fetchTeamMembers = async () => {
            try {
                ...
                const mentoringplans = await applyMentoring(member.teamId,
                    token);
    ...
    return (
        ...
        >
            <Box sx={style}>
                ...
                <S.Content>
                    <S.ContentText style={{ paddingTop: '10px' }}>멘토
                        현황</S.ContentText>
                    <div style={{
                        display: 'flex',
                        alignItems: 'center',
                        ...
                        {teamMembers
                        .filter(mentor => mentor.role
                            === 'Mentor') // role이 Mentor인 멤버만 필터링
                        .map((mentor, index) => (
                            <div key={index}>
                                <Box sx={{ flexGrow: 0 }}>
                                    <Tooltip title={mentor.nickname}>
                                        <IconButton sx={{ p: 0 }}>
                                            <Avatar
                                                alt={mentor.name}
                ...
                <S.BottomWrapper>
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <S.ContentData>{teaminfos.menteeCnt} /
                                4</S.ContentData>
                        </div>
                        <div style={{
                            display: 'flex',
